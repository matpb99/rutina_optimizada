...
def run_optimization(
    ...
    series = LpVariable.dicts("Series", [(e, d) for e in exercises for d in days], cat="Integer", lowBound=0, upBound=4)
    executed = LpVariable.dicts("Executed", [(e, d) for e in exercises for d in days], cat="Binary")
    ...
    for d in days:
        for group in set(exercises.values()):
            ...
            if group in group_per_day[d]:
                problem += lpSum(series[(e, d)] for e in exercises_in_group) <= max_series_per_group_per_day
    ...
    for group in set(exercises.values()):
        ...
        if group in exact_series_per_group and exact_series_per_group[group] > 0:
            problem += lpSum(series[(e, d)] for e in exercises_in_group for d in days) == exact_series_per_group[group]
    ...
    penalized = LpVariable.dicts("Penalized", [(e1, e2, d) for (e1, e2) in penalties for d in days], cat="Binary")
