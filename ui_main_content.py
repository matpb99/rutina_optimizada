...
def display_exercise_manager(all_muscle_groups):
    with st.container():
        ...
        st.write(
            "Filtra y selecciona los ejercicios a incluir en la rutina. Los ejercicios solo aparecerán si su grupo muscular fue seleccionado en la barra lateral."
        )
    ...
    with st.expander("➕ Añadir Nuevo Ejercicio"):
        ...
        if st.button("Añadir Ejercicio a la Lista", key="add_exercise_button"):
            if (
                new_exercise_name
                and new_exercise_group
                and new_exercise_group != "(Nuevo Grupo Muscular)"
            ):
                ...
                if is_new_group:
                    if (
                        new_exercise_group
                        not in st.session_state.selected_muscle_groups
                    ):
            ...
            exercises_data.append(
                {
                    ...
                    "Clave": name in st.session_state.get("key_exercises", []),
                }
            )

    if exercises_data:
        exercises_df = (
            pd.DataFrame(exercises_data)
            .sort_values(by="Grupo Muscular")
    ...
    if not edited_df.empty:
        selected_exercises_names = edited_df[edited_df["Seleccionar"]][
            "Ejercicio"
        ].tolist()
        ...
        exercises_for_optimization = {
            ...
            if name in st.session_state.exercises
            and st.session_state.exercises[name]
            in st.session_state.selected_muscle_groups
        }
    else:
        exercises_for_optimization = {}
        key_exercises = []
    ...
    with st.container():
        ...
        with st.expander("➕ Añadir Penalización"):
            ...
            if not available_exercises:
                ...
                with col3:
                    penalty_level = st.selectbox(
                        "Nivel de Penalización:",
                        options=list(PENALTY_LEVELS.keys()),
                        key="nivel_pen",
                    )
                ...
                if st.button("Añadir Penalización", key="add_penalty_button"):
                    if (
                        penalty_exercise_1
                        and penalty_exercise_2
                        and penalty_exercise_1 != penalty_exercise_2
                    ):
...
def display_results(problem, series, penalized):
    ...
    if LpStatus[problem.status] == "Optimal":
        ...
        for i in range(0, len(st.session_state.selected_weeks_day), 3):
            ...
            for j, d in enumerate(st.session_state.selected_weeks_day[i : i + 3]):
                with cols[j]:
                    st.markdown(
                        f"### 📅 **<span style='color:{DAY_COLORS.get(d, '#28a745')};'>Día: {d}</span>**",
                    ...
                    for e in st.session_state.exercises_for_optimization:
                        ...
                        if n and n > 0:
                            ...
                            exercise_group = (
                                st.session_state.exercises_for_optimization[e]
                            )
                    ...
                    if exercises_day_data:
                        ...
                        st.dataframe(
                            df_exercises, hide_index=True, use_container_width=True
                        )
                    else:
                    ...
                    if series_group_data:
                        ...
                        st.dataframe(
                            df_series_group, hide_index=True, use_container_width=True
                        )
                    else:
                        st.write(
        ...
        total_penalty_calc = lpSum(
            [
                ...
                if (e1, e2, d) in penalized
                and penalized[(e1, e2, d)].varValue is not None
            ]
        )
    ...
    st.divider()
    st.markdown(
        "**Nota:** Este optimizador utiliza programación lineal entera para encontrar la mejor combinación de ejercicios de acuerdo al número máximo de series por grupo muscular al día, el número máximo de series totales al día, los grupos musculares seleccionados para cada día, el número de series semanales totales por grupo muscular y los ejercicios que no pueden estar juntos el mismo día (penalizaciones)."
    )
