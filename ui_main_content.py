...
def display_exercise_manager(all_muscle_groups):
    ...
    with st.expander("➕ Añadir Nuevo Ejercicio"):
        new_exercise_name = st.text_input("Nombre del Ejercicio:", key="new_exercise_name")
        ...
        if new_exercise_group == "(Nuevo Grupo Muscular)":
            new_exercise_group_custom = st.text_input("Nombre del Nuevo Grupo Muscular:", key="new_exercise_group_custom")
        ...
        if st.button("Añadir Ejercicio a la Lista", key="add_exercise_button"):
            if new_exercise_name and new_exercise_group and new_exercise_group != "(Nuevo Grupo Muscular)":
                ...
                st.success(f"Ejercicio '{new_exercise_name}' añadido a '{new_exercise_group}'.")

                if is_new_group:
                    if new_exercise_group not in st.session_state.selected_muscle_groups:
                        st.session_state.selected_muscle_groups.append(new_exercise_group)
                    st.rerun()
            else:
                st.error("Por favor, introduce un nombre de ejercicio y un grupo muscular válido.")
    ...
    for name, group in st.session_state.exercises.items():
        if group in st.session_state.selected_muscle_groups:
            exercises_data.append({"Seleccionar": True, "Ejercicio": name, "Grupo Muscular": group, "Clave": False})
    
    if exercises_data:
        exercises_df = pd.DataFrame(exercises_data).sort_values(by="Grupo Muscular").reset_index(drop=True)
    else:
        exercises_df = pd.DataFrame(columns=["Seleccionar", "Ejercicio", "Grupo Muscular", "Clave"])
    ...
    with st.container():
        ...
        with st.expander("➕ Añadir Penalización"):
            ...
            if not available_exercises:
                st.info("Para añadir penalizaciones, primero selecciona los grupos musculares en la barra lateral.")
            else:
                col1, col2, col3 = st.columns(3)
                with col1:
                    penalty_exercise_1 = st.selectbox("Ejercicio 1:", options=available_exercises, key="ej1_pen")
                with col2:
                    penalty_exercise_2 = st.selectbox("Ejercicio 2:", options=available_exercises, key="ej2_pen")
                with col3:
                    penalty_level = st.selectbox("Nivel de Penalización:", options=list(PENALTY_LEVELS.keys()), key="nivel_pen")
                ...
                if st.button("Añadir Penalización", key="add_penalty_button"):
                    if penalty_exercise_1 and penalty_exercise_2 and penalty_exercise_1 != penalty_exercise_2:
                        key = tuple(sorted((penalty_exercise_1, penalty_exercise_2)))
                        st.session_state.penalties[key] = PENALTY_LEVELS[penalty_level]
                        st.success(f"Penalización añadida: {penalty_exercise_1} y {penalty_exercise_2} con nivel {penalty_level}.")
                        st.rerun()
                    else:
                        st.error("Por favor, selecciona dos ejercicios diferentes para la penalización.")
        ...
        if st.session_state.penalties:
            ...
            for (e1, e2), value in st.session_state.penalties.items():
                ...
                penalties_data.append({"Ejercicio 1": e1, "Ejercicio 2": e2, "Nivel": level})
...
def display_results(problem, series, penalized):
    ...
    if LpStatus[problem.status] == "Optimal":
        st.success(f"Se pudo encontrar una combinación válida 🎉")
        ...
        for i in range(0, len(st.session_state.selected_weeks_day), 3):
            ...
            for j, d in enumerate(st.session_state.selected_weeks_day[i:i+3]):
                with cols[j]:
                    st.markdown(f"### 📅 **<span style='color:{DAY_COLORS.get(d, '#28a745')};'>Día: {d}</span>**", unsafe_allow_html=True)
                    series_per_group_day = {group: 0 for group in st.session_state.exact_series_group.keys()}
                    total_series_day = 0
                    exercises_day_data = []
                    ...
                    for e in st.session_state.exercises_for_optimization:
                        ...
                        if n and n > 0:
                            exercises_day_data.append({"Ejercicio": e, "Series": int(n)})
                    ...
                    if exercises_day_data:
                        ...
                        st.dataframe(df_exercises, hide_index=True, use_container_width=True)
                    else:
                    ...
                    for group, s in series_per_group_day.items():
                        if s > 0:
                            series_group_data.append({"Grupo Muscular": group, "Series": s})
                    
                    if series_group_data:
                        df_series_group = pd.DataFrame(series_group_data)
                        st.dataframe(df_series_group, hide_index=True, use_container_width=True)
                    else:
                        st.write("No hay series asignadas a grupos musculares para este día.")

                    st.markdown(f"**💪 Total series del día: {total_series_day}**")
        ...
        st.metric(label="Total de Series en la Semana", value=total_series_global)
